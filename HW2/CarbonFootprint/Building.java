...
public class Building implements CarbonFootprint {
	...
	public Building(String id, int numberOfPeople, double electricity,
	...
	public String toString() {
		...
		if (getElectricity() > 0) {
			string.append("\nElectricity used (kWh): "
					+  output.toCommaNumber(getElectricity()));
		}
		if (getNaturalGas() > 0) {
			string.append("\nNatural Gas used (kWh): "
					+  output.toCommaNumber(getNaturalGas()));
		}
		if (getHeatingOil() > 0) {
			string.append("\nHeating Oil used (kWh): "
					+  output.toCommaNumber(getHeatingOil()));
		}
		if (getCoal() > 0) {
			string.append("\nCoal used (kWh): "
					+  output.toCommaNumber(getCoal()));
		}
		if (getLpg() > 0) {
			string.append("\nLPG used (kWh): "
					+  output.toCommaNumber(getLpg()));
		}
		if (getPropane() > 0) {
			string.append("\nPropane used (Gallons): "
					+  output.toCommaNumber(getPropane()));
		}
		if (getWoodenPellets() > 0) {
			string.append("\nWooden Pellets used (Metric Tons): "
					+  output.toCommaNumber(getWoodenPellets()));
		}
